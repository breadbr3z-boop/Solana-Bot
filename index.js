...
const bot = new TelegramBot(process.env.TELEGRAM_TOKEN, { polling: { params: { timeout: 10 }, interval: 300, autoStart: true } });
const jupiter = createJupiterApiClient(); 
const MY_ID = process.env.CHAT_ID;

// IDs for Legacy Raydium and the 2026 CPMM standard
const RAYDIUM_ID = new PublicKey('675kPX9MHTjS2zt1qfr1NYHuzeLXfQM9H24wFSUt1Mp8');
const RAYDIUM_CPMM_ID = new PublicKey('CPMMoo8LqacmJvYtSvpPbcS0_Dk686Fm_2T-L0k_68'); 
const SOL_MINT = "So11111111111111111111111111111111111111112";
let isPaused = false; 

// 2. COMMANDS & HEARTBEAT
bot.on('message', (msg) => {
...
setInterval(() => console.log(`💓 Alive: ${new Date().toLocaleTimeString()} | Monitoring 2 Engines`), 60000);
...
async function buyToken(mint) {
    ...
    while (Date.now() - start < 45000) {
        try {
            console.log(`📡 Jupiter Indexing... ${mint.slice(0, 5)}`);
...
// 5. THE DUAL-EYE SLEDGEHAMMER SCANNER
[RAYDIUM_ID, RAYDIUM_CPMM_ID].forEach(programId => {
    connection.onLogs(programId, async ({ logs, signature, err }) => {
        ...
        try {
            ...
            let mint = null;
            // Scan slots 8, 9, 7, 10, 6 for the Mint address
            for (let idx of [8, 9, 7, 10, 6]) {
                const addr = ix?.accounts[idx]?.toBase58();
                if (addr && addr.length > 30 && addr !== SOL_MINT && addr !== programId.toBase58()) {
                    mint = addr;
                    break;
                }
            }
            ...
            if (mint) {
                ...
                if (rug.data.score < 5000) {
                    ...
                    await buyToken(mint);
                    setTimeout(() => { isPaused = false; console.log("🔓 Ready."); }, 60000);
                }
            }
        } catch (e) { }
    }, 'processed');
