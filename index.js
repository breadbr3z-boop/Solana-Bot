...
setInterval(() => console.log("💓 Heartbeat: Deep-Scan Active"), 60000);

// 2. AUTO-SELL MONITOR (+50% / -30%)
async function monitorPrice(mint, entryPrice, tokens) {
    const watchdog = setInterval(async () => {
        try {
            const quote = await jupiter.quoteGet({ inputMint: mint, outputMint: SOL_MINT, amount: tokens.toString(), slippageBps: 50 });
            ...
            if (price >= entryPrice * 1.5 || price <= entryPrice * 0.7) {
                ...
                bot.sendMessage(MY_ID, `🎯 ${price >= entryPrice * 1.5 ? "TP" : "SL"} HIT! Sold: https://solscan.io/tx/${sig}`);
            }
        } catch (e) { }
    }, 20000);
}
...
connection.onLogs(RAYDIUM_ID, async ({ logs, signature, err }) => {
    ...
    try {
        ...
        let mint = null;
        [8, 9, 7, 10].forEach(idx => {
            const addr = ix?.accounts[idx]?.toBase58();
            if (addr && addr.length > 30 && addr !== SOL_MINT) mint = addr;
        });
...
console.log("🚀 FULL-POWER BOT LIVE.");
